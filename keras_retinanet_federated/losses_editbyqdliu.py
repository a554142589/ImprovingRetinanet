...
def focal_loss(y_true, y_pred):
    ...
    normalizer = backend.where(keras.backend.less(normalizer, 1.0), 1.0, normalizer)
...
def smooth_l1_loss(y_true, y_pred):
    ...
    normalizer = backend.where(keras.backend.less_equal(normalizer, 1.0), 1.0, normalizer)

    # assign_op = average.assign(0.99 * average.value() + 0.01 * normalizer)

    # with tf.control_dependencies([assign_op]):
    return keras.backend.sum(regression_loss) / normalizer
        # return 0.5 * keras.backend.sum(regression_loss) / normalizer
