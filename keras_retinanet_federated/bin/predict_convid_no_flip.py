#!/usr/bin/env python
"""
process a folder of nii s other than single one
"""
import argparse
import os
import sys
...
def parse_args(args):
    ...
    parser.add_argument('--detection-threshold', help='Threshold used for determining what detections to draw(0-1).', default=0.4, type=float)
    parser.add_argument('--segmentation-threshold',      help='Threshold used for filter segmentation map(0-1).', default=0.1, type=float)
    parser.add_argument('--attention-threshold', help='Threshold used for filter attention map.(0-1)', default=0.8, type=float)
...
def _get_detections(args, model, score_threshold=0.05, max_detections=100, source_path=None, save_path=None):
    ...
    nii_files = os.listdir(source_path)
    for nii_file in nii_files:
        filename = os.path.join(source_path, nii_file)
        ...
        print('Start Process {}'.format(nii_file))
        print('Volumes has {} slicecs in total'.format(slice_range))

        for i in tqdm(range(slice_range), desc='Processing {}:'.format(nii_file)):
            combined_img = np.zeros([512, 512, 3])
            if i == 0:
                raw_image = volume[i, :, :]
                im = np.int64(raw_image)
                im = format_image(scalar, multiplier, im)
                im = np.uint8(im)

                im_lower = volume[i + 1, :, :]
                im_lower = np.int64(im_lower)
                im_lower = format_image(scalar, multiplier, im_lower)
                im_lower = np.uint8(im_lower)

                combined_img[:, :, 0] = im
                combined_img[:, :, 1] = im
                combined_img[:, :, 2] = im_lower

            elif i == slice_range - 1:
                im_upper = volume[i - 1, :, :]
                im_upper = np.int64(im_upper)
                im_upper = format_image(scalar, multiplier, im_upper)
                im_upper = np.uint8(im_upper)
            ...
            # the read image seems flipped from nii
            # combined_img = cv2.flip(combined_img, 0)

            image = preprocess_image(combined_img.copy())
            ...
            attention_out[i,:,:] = cv2.resize(np.squeeze(np.uint8(attention_map * 255)), (origin_shape[1],origin_shape[0]))

            masks[masks < args.segmentation_threshold] = 0
            masks = cv2.resize(np.squeeze(np.uint8(masks * 255)), (origin_shape[1],origin_shape[0]))
            mask_out[i, :, :] = masks

        detection_out = sitk.GetImageFromArray(detection_out)
        detection_out.CopyInformation(sitk_img)
        sitk.WriteImage(detection_out, os.path.join(save_path, 'detection_' + str(nii_file)))

        attention_out = sitk.GetImageFromArray(attention_out)
        attention_out.CopyInformation(sitk_img)
        sitk.WriteImage(attention_out, os.path.join(save_path, 'attention_' + str(nii_file)))

        mask_out = sitk.GetImageFromArray(mask_out)
        mask_out.CopyInformation(sitk_img)
        sitk.WriteImage(mask_out, os.path.join(save_path, 'mask_' + str(nii_file)))
