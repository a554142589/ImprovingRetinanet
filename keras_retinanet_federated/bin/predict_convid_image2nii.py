...
def _get_detections(args, model, score_threshold=0.05, max_detections=100, source_path=None, save_path=None):
    ...
    imgs = sorted(imgs)
    detection_out = np.zeros([len(imgs), 512, 512, 3])
    attention_out = np.zeros([len(imgs), 512, 512])
    mask_out = np.zeros([len(imgs), 512, 512])
    for i in tqdm(range(len(imgs)), desc='Processing..'):
        ...
        if image.shape[0] !=512 or image.shape[1] !=512:
            raise ValueError("Image size should be 512x512")
        ...
        # run network
        # boxes, scores, labels, masks = model.predict_on_batch(np.expand_dims(image, axis=0))[:4]
    ...
    sitk.WriteImage(detection_out, os.path.join(save_path, 'detection_{}.nii.gz'.format(str(source_path.rsplit('/', -1)[-1]))))

    attention_out = sitk.GetImageFromArray(attention_out)
    sitk.WriteImage(attention_out, os.path.join(save_path, 'attention_{}.nii.gz'.format(source_path.rsplit('/', -1)[-1])))

    mask_out = sitk.GetImageFromArray(mask_out)
    sitk.WriteImage(mask_out, os.path.join(save_path, 'masks_{}.nii.gz'.format(str(source_path.rsplit('/', -1)[-1]))))
