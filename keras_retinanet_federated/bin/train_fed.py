...
def main(args=None, model_config=None):
    ...
    os.system('cp -r ./keras_retinanet/bin %s' % (args.snapshot_path+ './bin'))
    os.system('cp run.sh %s' % (args.snapshot_path))
    ...
    # data_list = ['train_volumes_segs_mixed_h5_relative_path.csv', 'train_volumes_segs_mixed_h5_relative_path.csv','train_volumes_segs_mixed_h5_relative_path.csv','train_volumes_segs_mixed_h5_relative_path.csv']
    # data_list = ['train_public_1.csv', 'train_public_2.csv','train_public_1.csv','train_public_2.csv']
    data_list = ['train_public_1.csv', 'train_public_2.csv']
    client_num = 2
    ...
    for i in range(client_num):
        args.annotations = os.path.join(anno_base_dir, data_list[i])
    ...
    for epoch in range(resume_epoch, args.epochs):
        ...
        for step in range(args.steps):
            ...
            for client_index in range(client_num):
                iterator = data_client[client_index]
            ...
            print("\rStep:[{}/{}] time:{:.2f}s  -loss:{:.4e} -classfication_loss:{:.4e} -regression_loss:{:.4e} -mask_loss:{:.4e}".format((step+1), args.steps, dur, total_loss/client_num/(step+1), classification_loss/client_num/(step+1), regression_loss/client_num/(step+1), mask_loss/client_num/(step+1)), end="", flush=True)
            ...
            if step % 20 == 0:
                ...
                for param_i in range(len(parameter_clients[0])):
                    new_parameter = (parameter_clients[0][param_i] + parameter_clients[1][param_i])/client_num
