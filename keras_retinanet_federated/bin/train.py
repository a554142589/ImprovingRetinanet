...
def create_callbacks(model, training_model, prediction_model, validation_generator, args, tensorboad_writer=None):
    ...
    if args.reduce_lr:
        # training_model.optimizer.lr *= 0.95
...
def main(args=None, model_config=None):
    ...
    anno_base_dir = '/research/dept8/qdou/covid/training_data/'
    ...
    for epoch in range(resume_epoch, args.epochs):
        ...
        for step in range(args.steps):
            ...
            print("\rStep:[{}/{}] time:{:.2f}s  -loss:{:.4e} -classfication_loss:{:.4e} -regression_loss:{:.4e} -mask_loss:{:.4e} - lr {:.4e}".format((step+1), args.steps, dur, total_loss/(step+1), classification_loss/(step+1), regression_loss/(step+1), mask_loss/(step+1), K.get_value(training_model.optimizer.lr)), end="", flush=True)
        print('\n', flush=True)
        # epoch end
