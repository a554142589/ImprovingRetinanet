...
class CSVGenerator(Generator):
    ...
    def have_mask(self, image_index):
        # path = self.image_path(image_index).replace('formatted_2mm_h5_Apr', 'formatted_mask').replace('h5', 'png')
    ...
    def load_mask(self, image_index):
        # path = 'masks/' + str(self.image_path(image_index).split('/', 1)[-1]).replace('h5', 'png') # update the path for masks
        # path = self.image_path(image_index).replace('formatted_2mm_h5_Apr', 'formatted_mask').replace('h5', 'png') ## changed by qdou on 15 Aug, use this line for DeepLesion
        ...
        # NOTE: when train on mined lesions or covid volumes, there is no mask. use a temp one.
        # path = '/research/dept6/yhlong/mrJiang/DeepLesionDataSet/formatted_mask/000001_01_01/109.png'
        # path = self.image_path(image_index).replace('images', 'masks').replace('h5', 'png')
        # print('******************** For debug ***** self.image_path(image_index) {}, path {}'.format(self.image_path(image_index), path))
        try:
            return np.expand_dims(np.asarray(Image.open(path)), axis=2)
        except FileNotFoundError:
            path = '../../COVIDDataSet/109.png'
            # path = '/research/dept6/yhlong/mrJiang/DeepLesionDataSet/formatted_mask/000001_01_01/109.png'
            # path = '/Users/jemary/Data/DataSet/COVID-19_CT_segmentation_mask_dataset/slice_0.png'
            return np.expand_dims(np.asarray(Image.open(path)), axis=2)
        # return np.expand_dims(np.asarray(Image.open(path)), axis=2)
