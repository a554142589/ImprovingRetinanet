...
def main(args=None, model_config=None):
    ...
    with open('deeplesion_load_weight_nl.txt','w') as f:
        print(model.weights, file=f)

    # print('Saving model')
    # file_path = os.path.join(args.snapshot_path, '{backbone}_{dataset_type}_01.h5'.format(backbone=args.backbone,
    #                                                                                              dataset_type=args.dataset_type))
    # if not os.path.exists(args.snapshot_path):
    #     os.makedirs(args.snapshot_path)
    # model.save('vgg19_csv_01.h5', overwrite=True)
    ...
    #         print("\rStep:[{}/{}] time:{:.2f}s  -loss:{:.4e} -classfication_loss:{:.4e} -regression_loss:{:.4e} -mask_loss:{:.4e}".format((step+1), args.steps, dur, total_loss/(step+1), classification_loss/(step+1), regression_loss/(step+1), mask_loss/(step+1)), end="", flush=True)
    #     print('\n', flush=True)
    #     # epoch end
