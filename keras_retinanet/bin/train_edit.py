...
def create_models(backbone_retinanet, num_classes, weights, multi_gpu=0, freeze_backbone=False, config=None, model_config=None):
    ...
    training_model.compile(
        ...
        optimizer=tf.keras.optimizers.Adam(lr=1e-4, clipnorm=0.001) # used to be 1e-4
    )
...
def main(args=None, model_config=None):
    ...
    for i in range(train_generator.size()):
        raw_image = train_generator.load_image(i)
        ...
        # run network
        boxes, labels, mask, attmap = model.predict_on_batch(np.expand_dims(image, axis=0))
        print(boxes.shape)
        print(labels.shape)
        print(mask.shape)
        print(attmap.shape)
        print(np.max(attmap))
        print(np.min(attmap))
        # cv2.imwrite('mask.png',np.squeeze(mask))
        cv2.imwrite('p2_raw{}.png'.format(i),raw_image)
        cv2.imwrite('p2_attmap{}.png'.format(i),cv2.resize(np.squeeze(np.uint8(attmap*255)),(512,512)))
        break
    # data = train_generator.next()
    # data = tf.convert_to_tensor(data)
    # outputs = training_model(data)
    # for out in outputs:
    #     print(out.shape)
    # start training
